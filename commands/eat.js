...
module.exports = {
  ...
  run: async (message, args, client) => {
    if (message.channel.name == "priv-cannibal") {
      ...
      if (!message.member.roles.cache.has(client.config.ids.alive)) return message.channel.send(`Eating when dead just doesn't make any sense`)
      ...
      for (let i = 0 ; i < args.length ; i++) {
        ...
        if (!guy.roles.cache.has(client.config.ids.alive)) return message.channel.send(`Player **${guy.nickname} ${guy.user.username}** is dead!`)
