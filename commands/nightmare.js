...
module.exports = {
    ...
    run: async (message, args, client) => {
        if (message.channel.name == "priv-nightmare-werewolf") {
            ...
            if (!message.member.roles.cache.has(client.config.ids.alive)) return message.chanenl.send("Ok when you are dead, the only nightmares you're getting are from hell coz you're a bad bad wolf.")
            ...
            if (!guy.roles.cache.has(client.config.ids.alive)) return message.channel.send("That player is already dead... Have some decency bro.")
