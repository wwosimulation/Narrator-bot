            ...
            if(args) {
                if(args[0] === "here" && args[1]) {
                    ...
                    args.slice(1).forEach((arg) => {
                        ...
                        if(cmdManager.cache.has(arg)) cmdManager.cache.get(arg).delete().then((cmd) => answer += `Delete \`${cmd.name}\` (\`${cmd.id}\`, \`${cmd.guild.name}\`)\n`)
                        else if(cmdManager.cache.find((cmd) => cmd.name === arg).size == 1) cmdManager.cache.find((cmd) => cmd.name === arg).delete().then((cmd) => answer += `Delete \`${cmd.name}\` (\`${cmd.id}\`, \`${cmd.guild.name}\`)\n`)
                    ...
                    args.forEach((arg) => {
                        ...
                        // Nuking command IDs and names
                        s.forEach((ser) => {
                            if(client.guilds.resolve(ids[ser]).commands.cache.has(arg)) client.guilds.resolve(ids[ser]).commands.cache.get(arg).delete().then((cmd) => answer += `ID delete \`${cmd.name}\` (\`${cmd.id}\`, \`${cmd.guild.name}\`)\n`)
                            else if(client.guilds.resolve(ids[ser]).commands.cache.find((cmd) => cmd.name === arg).size == 1) client.guilds.resolve(ids[ser]).commands.cache.find((cmd) => cmd.name === arg).delete().then((cmd) => answer += `Name delete \`${cmd.name}\` (\`${cmd.id}\`, \`${cmd.guild.name}\`)\n`)
                            else skiped.push(arg)
                        })
                    })
                }
