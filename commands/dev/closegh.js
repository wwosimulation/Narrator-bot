module.exports = {
    ...
    run: async (message, args, client) => {
        ...
        let issue = await client.github.request("GET /repos/{owner}/{repo}/issues/{issue_number}", {
            owner: "wwosimulation",
            repo: "tracker",
            issue_number: args[0]
        }).catch(e => e)
        ...
        let user = issue.data.body.split("\n").find(r => r.startsWith("**User ID:**"))?.split(":**")[1].trim()
        ...
        if (args.length > 1) {
            comment = await client.github.request("POST /repos/{owner}/{repo}/issues/{issue_number}/comments", {
                owner: "wwosimulation",
                repo: "tracker",
                issue_number: args[0],
        ...
        let closedIssue = await client.github.request("POST /repos/{owner}/{repo}/issues/{issue_number}", {
            owner: "wwosimulation",
            repo: "tracker",
            issue_number: args[0],
            state: "closed"
