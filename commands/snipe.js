...
module.exports = {
    ...
    run: async (message, args, client ) => {
        if (message.channel.name === "priv-sheriff") {
            ...
            if (!message.member.roles.cache.has(client.config.ids.alive)) return message.channel.send("THE NEXT TIME YOU TRY TO BREAK ME, YOU WILL SEE YOUR NAME IN <#>.")
            ...
            if (!guy.roles.cache.has(client.config.ids.alive)) return message.channel.send("As Sheriff, you need to CHECK a player that is ALIVE and they must be DEAD when DAY STARTS. Hope you can understand English moron.")
