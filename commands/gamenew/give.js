...
module.exports = {
    ...
    run: async (message, args, client) => {
        ...
        if (player.role === "Alchemist") {
            ...
            await message.channel.send(`${getEmoji(`${itemType}p`, client)} You have given your ${itemType} potion to **${players.indexOf(target)+1} ${db.get(`player_${target}`).user.username}**. ${additionalMsg}`)
        }

        if (player.role === "Forger") {
            db.set(`player_${player.id}.target`, target)
            await message.channel.send(`${getEmoji(`get${player.itemType}`, client)} You have decided to give your forged ${player.itemType} to **${players.indexOf(target)} ${db.get(`player_${target}`).user.username}**.`)
        }
