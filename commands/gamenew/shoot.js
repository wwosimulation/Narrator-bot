...
module.exports = {
    ...
    run: async (message, args, client) => {
        ...
        if (gamePhase % 3 === 0 && ["Gunner", "Marksman"].includes(player.role)) return await message.channel.send("You do know that you can only shoot during the night right? Or are you delusional?")
        ...
        let messages = {
            ...
            "Jailer": `${getEmoji("bullet", client)} The Jailor executed their prisoner last night! **${players.indexOf(target)+1} ${db.get(`player_${target}`).username} (${getEmoji(db.get(`player_${target}`).role.toLowerCase().replace(/\s/g, "_"), client)} ${db.get(`player_${target}`).role})** is dead!`,
