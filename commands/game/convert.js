...
module.exports = {
    ...
    run: async (message, args, client) => {
        ...
        if (!player.hypnotized) {
            if (player.id === target) return await message.channel.send("You cannot convert yourself lmao!")
        }
        ...
        if (player.role === "Zombie") {
            ...
            message.guild.channels.cache.find((c) => c.name === "zombies-chat")?.send(`${getEmoji("zombvote", client)} **${players.indexOf(player.id) + 1} ${player.username}** selected to bite **${players.indexOf(target) + 1} ${db.get(`player_${target}`).username}**.`)
        } else {
            message.channel.send(`${getEmoji(player.role === "Bandit" ? "kidnap" : "sect_member", client)} You have decided to convert **${players.indexOf(target) + 1} ${db.get(`player_${target}`).username}** into your ${player.role === "Bandit" ? "accomplice" : "sect"}.`)
        }
