    ...
    run: async (message, args, client) => {
        ...
        if (message.channel.name.startsWith("priv")) {
            ...
            if (["Medium", "Ritualist"].includes(player.role) || ["Medium", "Ritualist"].includes(player?.dreamRole)) {
                ...
                allDeads.forEach((p) => {
                    if (!p.jailed && !p.nightmared && !p.hypnotized) {
                        message.guild.channels.cache.get(p.channel).send(`${getEmoji(player.role.toLowerCase().replace(/\s/g, "_"), client)} ${-players.indexOf(message.author.id) + 1} ${player.username}: ${content}`)
                    }
                })
            }
        } else {
            switch (message.channel.name) {
                case "dead-chat":
                    ...
                    ps.forEach((p) => {
                        if (!p.hypnotized && !p.jailed && !p.nightmared) {
