                ...
                if (!guy.roles.cache.has(alive.id) || !ownself.roles.cache.has(alive.id)) {
                    ...
                    if (priest != null) {
                        ...
                        if (dayCount == 1) {
                            ...
                            if (sectMembers.find((m) => m.id === message.author.id) && db.get(`role_${guy.id}`) === "Sect Leader") return message.channel.send("You can not splash the leader of the sect if you are sected!")
                            ...
                            for (let x = 0; x < cupid.length; x++) {
                                ...
                                if (message.author.nickname === couple[0]) {
                                    if (!sectMembers.find((m) => m.id === message.author.id) && guy.nickname === couple[1]) return message.channel.send("You can not splash your lover!")
                                }
