...
module.exports = {
    ...
    run: async (message, args, client) => {
        ...
        if (buy.includes("roses")) {
            ...
            let price = 500
            let allsprole = message.guild.roles.cache.filter(r => r.postion < 184 && r.position > 63).keyArray("id")
            let hassprole = false
            allsprole.forEach(e => {
                if (client.guilds.cache.get("465795320526274561").members.cache.get(message.author.id).roles.cache.has(e)) {
                    hassprole = true
