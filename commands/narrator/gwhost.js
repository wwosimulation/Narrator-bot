...
module.exports = {
    ...
    run: async (message, args, client) => {
        ...
        let button = new client.disbut.MessageButton().setStyle("green").setLabel("Join Game").setID("gwjoin")
        let m = await message.guild.channels.cache.get("606123818305585167").send(`We are now starting game ${args.join(" ")}. Our host will be <@${message.author.id}>! To join the game, react with :fries:. If you do not wish to get future pings about the game, go to <#606123783605977108> and react with 🎮${sup ? `\n\n${sup}` : ""}`, button)
        //await m.react("🍟")
        db.set(`game`, m.id)
        ...
        //             .find((x) => x.name == "joined-link")
        //             .send(`<@${guy.id}>, use the link above to join the game!`)
