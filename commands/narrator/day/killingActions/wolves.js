...
module.exports.wolves = async (client, alivePlayersBefore) => {
    ...
    let mappedWolf = alivePlayers.filter((a) => db.get(`player_${a}`).role?.toLowerCase().includes("wolf")).map((a) => [a, db.get(`player_${a}`).role]) // fillter the wolves and check if there are any
    ...
    if (totalVotes.length > 0) {
        ...
        if (totalVotes[0][1] === totalVotes[1][1]) {
            ...
            if (sortedWolves[0] === sortedWolves[1]) {
                ...
                toKill = Object.entries(votes)
                    .filter((v) => strongWolves.indexOf(db.get(`player_${v[0]}`).role) === sortedWolves[0])
                    .pop()[1] // gets the last strongest werewolf who voted
            } else {
                // get the voted player by the strongest wolf
                toKill = Object.entries(votes)
                    .filter((v) => db.get(`player_${v[0]}`).role === strongWolves[sortedWolves[0]])
        ...
        if (guy.status === "Alive") {
            ...
            if (kwwDied === true || guy.role === "Cursed" || guy.role === "Werewolf Fan") {
                ...
                if (guy.team !== "Village" || headhunterTargets.includes(guy.id) || typeof guy.sected === "string" || guy.bitten === true || ["Bandit", "Corruptor", "Cannibal", "Illusionist", "Serial Killer", "Arsonist", "Bomber", "Alchemist", "Hacker", "Dreamcatcher"].includes(guy.role)) {
