...
module.exports.wolves = async (client, alivePlayersBefore) => {
    ...
    let mappedWolf = alivePlayersBefore.filter(a => db.get(`player_${a}`).role?.toLowerCase().includes("wolf") && db.get(`player_${a}`).role !== "Werewolf Fan")
    ...
    if (totalVotes.length > 0) {
      ...
      if (totalVotes.length > 1 && totalVotes[0][1] === totalVotes[1][1]) {
        ...
        if (sortedWolves.length > 1 && sortedWolves[0][0] === sortedWolves[1][0]) {
          ...
          toKill = Object.entries(votes).filter(v => strongWolves.indexOf(db.get(`player_${v[0]}`).role) === sortedWolves[0][0]).pop()[1] // gets the last strongest werewolf who voted
        } else {
          // get the voted player by the strongest wolf
          toKill = Object.entries(votes).filter(v => db.get(`player_${v[0]}`).role === sortedWolves[0][1]).pop()[1] // gets the strongest werewolf who voted
        }
	...
	toKill = totalVotes[0][0]
      ...
      if (guy.status === "Alive") {
        ...
        if (kwwDied === true || guy.role === "Cursed" || guy.role === "Werewolf Fan") {
          ...
          if ((guy.team !== "Village" && guy.role !== "Werewolf Fan" ) || headhunterTargets.includes(guy.id) || typeof guy.sected === "string" || guy.bitten === true || ["Bandit", "Corruptor", "Cannibal", "Illusionist", "Serial Killer", "Arsonist", "Bomber", "Alchemist", "Hacker", "Dreamcatcher", "Wise Man"].includes(guy.role)) { // check if the player is not from the village
