...
module.exports = {
    ...
    run: async (message, args, client) => {
        ...
        let m = await message.guild.channels.cache.get("860552178095882240").send({ content: `, we are now starting game RS.${rs}[${args.join(" ")}]. Our host will be <@${message.author.id}>!\nIf you do not wish to get future pings about the game, go to <#862712560511221791> and react with 🏆`, embeds: [embed], components: [row] })
        db.set(`game`, m.id)
