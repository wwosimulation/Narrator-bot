...
module.exports = {
	...
	wolves.forEach(async wolf => { await message.guild.channels.cache.find(x => x.name === "ww-vote").permissionOverwrites.edit(wolf, { SEND_MESSAGES: false, VIEW_CHANNEL: true, READ_MESSAGE_HISTORY: true }) })
        zombies.forEach(async zomb => { await zombchat.permissionOverwrites.edit(zomb, { SEND_MESSAGES: true, VIEW_CHANNEL: true, READ_MESSAGE_HISTORY: true }) })
        siblings.forEach(async sib => { await sibchat.permissionOverwrites.edit(sib, { SEND_MESSAGES: true, VIEW_CHANNEL: true, READ_MESSAGE_HISTORY: true }) })
