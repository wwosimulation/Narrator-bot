...
module.exports = {
    ...
    run: async (message, args, client) => {
        ...
        if (message.channel.name == "priv-sect-leader") {
            if (!message.member.roles.cache.has(client.config.ids.alive)) return message.channel.send("You have a yarn for a brain")
            ...
            if (!guy.roles.cache.has(client.config.ids.alive) || !message.member.roles.cache.has(client.config.ids.alive)) return message.channel.send("Bruh, you or your target is not alive!")
            ...
            for (let i = 0 ; i < allbandits.length ; i++) {
                ...
                if (chan.permissionsFor(ownself).has(["SEND_MESSAGES", "VIEW_CHANNEL", "READ_MESSAGE_HISTORY"])) {
                    ...
                    for (let j = 1 ; j <= alive.members.size + dead.members.size ; j++) {
                        ...
                        if (player.roles.cache.has(client.config.ids.alive) && player.id != ownself.id) {
            ...
            if (!message.member.roles.cache.has(client.config.ids.alive)) return message.channel.send("You aren't alive! Now here's a great idea: Act like a medium or Wolf Medium as you may, and try reviving wolves...")
            ...
            if (!guy.roles.cache.has(client.config.ids.alive)) return message.channel.send("Stop trying to convert dead players already. They're dead. Leave them alone...")
            ...
            if (!message.member.roles.cache.has(client.config.ids.alive)) return message.channel.send("Try to look at yourself first before you judge others...")
            ...
            if (!guy.roles.cache.has(client.config.ids.alive)) return message.channel.send("BRUH YOU CAN'T BITE DEAD PLAYERS DUMB")    
