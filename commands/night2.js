...
module.exports = {
  ...
  run: async (message, args, client) => {
    ...
    // -----
    // decide player getting lynched
    let allvotes = [] //stores votes from players
    let shadowEnabled = db.get(`vtshadow`) // checks from DB if shadow wolf use ability
    ...
    for (let j = 1; j <= alive.members.size + dead.members.size; j++) { //iterate through all players
        ...
        if (tempguy && tempguy.roles.cache.has(alive.id)) { // if player is alive and is in server
            ...
            if ((shadowEnabled == true) && (db.get(`role_${tempguy.id}`).toLowerCase.includes("wolf") || db.get(`role_${tempguy.id}`) == "Sorcerer")) {
                if (guy_voted != "0") {
                        allvotes.push(guy_voted)
                        allvotes.push(guy_voted)
                    }
            } 
            // double votes for mayor if voted
            else if ((db.get(`mayorrev_${tempguy.id}`) == true) && guy_voted != "0") { //(db.get(`role_${tempguy.id}`) == "Mayor") && 
                allvotes.push(guy_voted)
                allvotes.push(guy_voted)
            } 
            // save vote for rest if voted
            else if(guy_voted != "0") {
                allvotes.push(guy_voted)
            }
        }
    ...
    for (let i = 0; i < vote_list.length; i++){
        if(!vote_list[i]){
        vote_list[i] = 0 // mark undefined places as 0
        }
    }
    ...
    console.log("Votes for all players: " + vote_list)
    console.log("Tie: " + tie)
