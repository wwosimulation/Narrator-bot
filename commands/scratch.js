...
module.exports = {
    ...
    run: async (message, args, client) => {
        if (message.channel.name == "priv-kitten-wolf") {
            ...
            if (!message.member.roles.cache.has(client.config.ids.alive)) return message.channel.send("WOW GUESS WHAT! There is a Wolf Medium just waiting to revive you! Just do `CTRL` `W` and you will get revived!")
            ...
            if (!guy.roles.cache.has(client.config.ids.alive)) return message.channel.send("Converting dead players is all the rage isn't it? You are really smart, A-word hole.")
