        ...
        if (lotteries.length != 0) {
            ...
            if (new Date().getTime() > lot.endDate) {
                ...
                if (lot.participants.length == 0) {
                    ...
                    if (!client.guilds.resolve(ids.server.sim).members.cache.get(winner.id).roles.cache.has("947629828771831888")) client.guilds.resolve(ids.server.sim).members.cache.get(winner.id).roles.add("947629828771831888")
                    let part = []
