    ...
    async function createCollector(msg, action) {
        await msg
            ...
            .then(async (i) => {
                ...
                if (action === "redirect") await i.update({ content: "Done!", components: [] })
                if (action === "redirect") await interaction.channel.send({ content: `${getEmoji("flagger_protect", client)} You have decided to protect **${players.indexOf(flagger.target) + 1} ${db.get(`player_${flagger.target}`).username}** by redirecting attacks to **${players.indexOf(flagger.redirect) + 1} ${db.get(`player_${flagger.redirect}`).username}**!` })
            })
            .catch(() => null)
    }
}
